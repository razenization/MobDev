...
const getItem = (data, index) => {
  return {
    id: `${data[index].id || data[index].isbn13}`,
...
export default function MoviesScreen() {
  ...
  useEffect(() => {
    connection.transaction((tx) => {
      tx.executeSql(
        'create table if not exists books (id integer primary key not null, title text, subtitle text, price real, image text, url text);',
        null,
      );
    });
  });
  ...
  const fetchBooks = async (search) => {
    if (search.length >= 3) {
      ...
      try {
        ...
        if (!result.length) {
          setIsLoading(false);
          return setError('No books have been found for your query.');
        }

        console.log('result => ', result);

        const dbValues = result
          .map(
            (book) =>
              `('${book.isbn13}', '${book.title}', '${
                book.subtitle
              }', '${book.price.slice(1)}', '${book.image}', '${book.url}')`,
          )
          .join(', ');
        ...
        await new Promise((resolve, reject) => {
          connection.transaction((tx) => {
            tx.executeSql(
              `insert into books (id,title,subtitle,price,image,url) values ${dbValues}`,
              [],
              (_, { rows }) => {
                const books = rows._array;

                const filteredByQuery = books.filter(
                  (book) =>
                    book.title.toLowerCase().indexOf(search.toLowerCase()) >
                      -1 ||
                    book.subtitle.toLowerCase().indexOf(search.toLowerCase()) >
                      -1,
                );
